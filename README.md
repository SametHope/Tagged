...
## Tagged API
Static methods are present on `Tagged` to interact with `ITagged` implementors, Unity objects and even regular objects.  
...
If a call fails due to `target` or `tags` being null, it will silently fail and return false.
...
These extensions can be disabled by adding `DISABLE_TAGGED_EXTENSIONS` preprocessor directive to the player options on the project settings.  
...
## Custom Tags
It is possible to use Tagged without using any Unity tags as you can add and remove any string as a tag and use it as such. In fact, `Tagged` supports displaying and editing such tags on the inspector with the `TaggedAttribute`.  
